...
const ZONE_BIZ_FULL_KEY = "abox.agent.zoneBizFull.v1";
...
export function removeZone(zoneId) {
  ...
  // Drop the full analysis cache for this zone.
  const fulls = safeGet(ZONE_BIZ_FULL_KEY, {});
  if (fulls[zoneId]) {
    const { [zoneId]: _dropFull, ...restFull } = fulls;
    safeSet(ZONE_BIZ_FULL_KEY, restFull);
  }
...
// --- Full business-analysis cache ---
//
// The slim summary above is enough for the dashboard's count chips, but the
// Working Areas map needs the WHOLE result (gold/silver/bronze with street
// geometry, recommendations, agencies, report) to re-draw polylines, stars,
// and competitor pins after a navigation. We persist it here so coming back
// to Working Areas restores the analysis automatically — until the user
// explicitly clears it from the Business drawer.
//
// Shape: { [zoneId]: <full result object as held in component state> }
//
// localStorage has a ~5 MB budget; if quota is hit we fail silently (the
// session-state copy in React still works for the rest of this tab session).

export function loadZoneBusinessFull() {
  return safeGet(ZONE_BIZ_FULL_KEY, {});
}

export function saveZoneBusinessFull(zoneId, result) {
  const all = loadZoneBusinessFull();
  const next = { ...all, [zoneId]: result };
  safeSet(ZONE_BIZ_FULL_KEY, next);
  return next;
}

export function clearZoneBusinessFull(zoneId) {
  const all = loadZoneBusinessFull();
  if (!all[zoneId]) return all;
  const { [zoneId]: _drop, ...rest } = all;
  safeSet(ZONE_BIZ_FULL_KEY, rest);
  return rest;
}
...
function hydrateICase(c) {
  return {
    ...
    customerKeys: [],
    selectedPropertyIds: [],
    rules: [],          // legacy rule list (kept for backward compat)
    // New node-based automation flow:
    //   { nodes: [{ id, type, kind, position:{x,y}, data, refId? }, ...],
    ...
    createdAt: 0,
    ...c,
    // Re-apply nested defaults in case the merge clobbered them with partials
    flow: {
      nodes: [],
      edges: [],
      ...(c?.flow || {}),
